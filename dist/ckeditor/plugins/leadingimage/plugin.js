...
( function() {
	...
	CKEDITOR.plugins.add( 'leadingimage', {
		lang: 'en,zh-cn',
		...
		init: function( editor ) {
			...
			// Register the widget.
			editor.widgets.add( 'leadingimage', image );

			// Add toolbar button for this plugin.
			editor.ui.addButton && editor.ui.addButton( 'LeadingImage', {
                icon: 'image',
				label: editor.lang.common.image,
				command: 'leadingimage',
				toolbar: 'insert,10'
			} );
			...
			CKEDITOR.dialog.add( 'leadingimage', this.path + 'dialogs/leadingimage.js' );
		},
	...
	function widgetDef( editor ) {
		var alignClasses = editor.config.leadingImage_alignClasses,
			captionedClass = editor.config.leadingImage_captionedClass;
		...
		function inflate() {
			...
			if ( this.deflated ) {
				this.widget = editor.widgets.initOn( this.element, 'leadingimage', this.widget.data );
		...
		return {
			...
			init: function() {
				var helpers = CKEDITOR.plugins.leadingimage,
				...
				if ( editor.filter.checkFeature( this.features.dimension ) && editor.config.leadingImage_disableResizer !== true )
					setupResizer( this );
			...
			// Overrides default method to handle internal mutability of leadingimage.
	 ...
	 * @singleton
	 */
	CKEDITOR.plugins.leadingimage = {
		stateShifter: function( editor ) {
			// Tag name used for centering non-captioned widgets.
			var doc = editor.document,
				alignClasses = editor.config.leadingImage_alignClasses,
				captionedClass = editor.config.leadingImage_captionedClass,
				...
				hasCaption:	function( shift, oldValue, newValue ) {
					...
					if ( newValue ) {
						...
						var figure = CKEDITOR.dom.element.createFromHtml( templateBlock.output( {
							...
							captionPlaceholder: editor.lang.leadingimage.captionPlaceholder
						} ), doc );
				...
				link: function( shift, oldValue, newValue ) {
					if ( shift.changed.link ) {
						...
						if ( !newValue )
							...
							var attributes = CKEDITOR.plugins.leadingimage.getLinkAttributesGetter()( editor, newValue );
	...
	function centerWrapperChecker( editor ) {
		var captionedClass = editor.config.leadingImage_captionedClass,
			alignClasses = editor.config.leadingImage_alignClasses,
	...
	function setupResizer( widget ) {
		...
		resizer.setAttribute( 'title', editor.lang.leadingimage.resizer );
		...
		return function( value ) {
			...
			command.on( 'refresh', function( evt ) {
				...
				if ( enabled === undefined )
					enabled = editor.filter.checkFeature( editor.widgets.registered.leadingimage.features.align );
			...
			if ( dialog.name == 'link' ) {
				...
				def.onShow = function() {
					...
					if ( widget && ( widget.inline ? !widget.wrapper.getAscendant( 'a' ) : 1 ) ) {
						...
						// Hide the display text in case of linking leadingImage widget.
						displayTextField.hide();
					} else {
						// Make sure that display text is visible, as it might be hidden by leadingImage integration
						// before.
	...
	function getFocusedWidget( editor ) {
		...
		if ( widget && widget.name === 'leadingimage' )
			return widget;
	...
	// on configurations like config#leadingImage_alignClasses or
	// config#leadingImage_captionedClass.
	//
	// @param {CKEDITOR.editor}
	// @returns {Object}
	function getWidgetAllowedContent( editor ) {
		var alignClasses = editor.config.leadingImage_alignClasses,
			rules = {
				...
				figure: {
					classes: '!' + editor.config.leadingImage_captionedClass
				},
				figcaption: true
			};
 ...
 *		config.leadingImage_captionedClass = 'captionedImage';
 *
 * @cfg {String} [leadingImage_captionedClass='image']
 * @member CKEDITOR.config
 */
CKEDITOR.config.leadingImage_captionedClass = 'image';
 ...
 * [SDK sample](http://sdk.ckeditor.com/samples/captionedimage.html).
 *
 *		config.leadingImage_prefillDimensions = false;
 *
 * @since 4.5
 * @cfg {Boolean} [leadingImage_prefillDimensions=true]
 * @member CKEDITOR.config
 */
 ...
 * [SDK sample](http://sdk.ckeditor.com/samples/captionedimage.html).
 *
 *		config.leadingImage_disableResizer = true;
 *
 * @since 4.5
 * @cfg {Boolean} [leadingImage_disableResizer=false]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.leadingImage_alignClasses = [ 'align-left', 'align-center', 'align-right' ];
 *
 ...
 *
 * @since 4.4
 * @cfg {String[]} [leadingImage_alignClasses=null]
 * @member CKEDITOR.config
 */
 ...
 *
 *		config.leadingImage_altRequired = true;
 *
 * Read more in the [documentation](#!/guide/dev_captionedimage) and see the
 * [SDK sample](http://sdk.ckeditor.com/samples/captionedimage.html).
 *
 * @since 4.6.0
 * @cfg {Boolean} [leadingImage_altRequired=false]
 * @member CKEDITOR.config
 */
