...
CKEDITOR.dialog.add('leadingimage', function (editor) {
    ...
    var regexGetSizeOrEmpty = /(^\s*(\d+)(px)?\s*$)|^$/i,
        ...
        helpers = CKEDITOR.plugins.leadingimage,

        // Editor instance configuration.
        config = editor.config,

        uploadUrl = config.filebrowserImageUploadUrl || config.filebrowserUploadUrl,
        downloadUrl = config.filebrowserImageDownloadUrl || config.filebrowserDownloadUrl,

        hasFileBrowser = false,

        // Content restrictions defined by the widget which
        // impact on dialog structure and presence of fields.
        features = editor.widgets.registered.leadingimage.features,
    ...
    function onChangeSrc() {
        ...
        if (value !== widget.data.src) {
            ...
            preLoader(value, function (image, width, height) {
                ...
                widthField.setValue(editor.config.leadingImage_prefillDimensions === false ? 0 : width);

                // Fill height field with the height of the new image.
                heightField.setValue(editor.config.leadingImage_prefillDimensions === false ? 0 : height);
    ...
    var pageContents = [
        {
            ...
            elements: [
                ...
                },
                {
                    ...
                    validate: editor.config.leadingImage_altRequired === true ? CKEDITOR.dialog.validate.notEmpty(lang.altMissing) : null
                },
                {
                    type: 'hbox',
                    widths: ['25%', '25%', '50%'],
            ...
            elements: [
                {
                    ...
                    onLoad: function () {
                        var template = "<input type='file' style='display: none;' id='file_" + this.domId + "'/>";
                        $("#" + this.domId).parent().append(template);
                        fileElementId = this.domId;
                    },
                    onClick: function () {
                        $("#file_" + this.domId).click();
                    }
                },
                {
                    id: 'uploadButton',
                    ...
                    onClick: function () {
                        ...
                        fd.append("upload", 1);
                        /* 把文件添加到表单里 */
                        fd.append("upfile", $("#file_" + fileElementId)[0].files[0]);
                        ajax.open("post", uploadUrl, true);

                        ajax.onload = function () {
                            var result = JSON.parse(ajax.responseText);
                            if (result && "success" === result.status) {
                                var fileId = (result.data && result.data.length) ? result.data[0].id : result.data.id;
                                setFileSrc(downloadUrl + "/" + fileId + "?type=original");
                            }
                        };
                        ajax.send(fd);
                    }
    ...
    function setFileSrc(src) {
        ...
        preLoader(src, function (image, width, height) {
            // Re-enable width and height fields.
            toggleDimensions(true);
